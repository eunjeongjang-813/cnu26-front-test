...
| `middleware.ts` | 요청 전처리                      | 프로젝트 루트에 위치     |
...
export async function POST(request: NextRequest) {
  ...
  // /orders 페이지의 캐시를 무효화 → 다음 방문 시 새 데이터 패칭
...
### 기초: 실습 코드 — middleware.ts

```ts
// middleware.ts — 프로젝트 루트 (실습 파일)
...
// Middleware에서 쿠키 읽기 (request 객체에서)
export function middleware(request: NextRequest) {
  const token = request.cookies.get("token")?.value;
  // ...
}
...
| [실습 2]  | `middleware.ts`                | 쿠키 읽기 + 리다이렉트               |
...
| Middleware       | 요청 전 실행. 쿠키 확인 + 리다이렉트         | middleware.ts            |
